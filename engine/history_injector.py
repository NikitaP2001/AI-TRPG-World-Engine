"""Generic history injector — marker-based context delta injection.

Moved from gm/history_injector.py — agnostic of any agent type.
"""
...
class HistoryInjector:
    ...
    def ensure_story_summaries(self, *, world: World) -> None:
        """Recover paragraph/arc summary deltas if they were trimmed out."""
        try:
        ...
        for arc in reversed(arcs):
            ...
            for para in paragraphs:
                ...
                loc_common = self._location_common_top_level(world, locations)
            ...
            parts = [f'Arc finalized: "{arc_name}"']
    ...
    def inject_paragraph_summary(
        ...
        *,
        ...
        parts = [f'Story paragraph completed: "{nm}"']
